...
import { assistantFollowUps, InsertAssistantFollowUp, InsertProjectInquiry, InsertUser, projectInquiries, users } from "../drizzle/schema";
...
export async function createProjectInquiry(inquiry: InsertProjectInquiry): Promise<void> {
  const db = await getDb();
  if (!db) {
...
export async function createAssistantFollowUp(followUp: InsertAssistantFollowUp): Promise<void> {
  const db = await getDb();
  if (!db) {
...
export async function listAssistantFollowUps() {
  const db = await getDb();
